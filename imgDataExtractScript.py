...
# ********************** Correlation Matrices - Visualizations ***********************************************
# Inter-Variable Correlation Matrix
df = pd.read_excel("C:/Screenshot_Network_Location/ExcleFileName_WhereBodyData_WiilBeSaved.xlsx")
...
# from ArboLeaf app's screenshots.
...
# Generates a correlation matrix visualizing the relationships between all collected body statistic 
# pairs from ArboLeaf app's screenshots, filtering for those pairs with correlations above a given threshold

# Sets a correlation threshold (e.g., 0.8)
corr_threshold = 0.8

# Uses np.triu to obtain the upper triangle of the symmetric matrix (avoid duplicates)
upper = corr_matrix.where(np.triu(np.ones(corr_matrix.shape), k=0).astype(bool))

# Keeps variable pairs with an absolute correlation above threshold
to_keep = [column for column in upper.columns if any(abs(upper[column]) >= corr_threshold)]

# Creates a new DataFrame to store the plot-qualifying pair of body statisitc variables 
corr_pairs_df = df[to_keep]

# Plots the correlations using Seaborn
plt.figure(figsize=(10, 8))  # Adjust figure size as needed
sns.heatmap(corr_pairs_df.corr(), annot=True, cmap="coolwarm", fmt=".2f")
plt.title("Correlation Matrix - Variables with corr coef >= corr_threshold")
plt.xticks(rotation=45, ha='right')
plt.show()
...
fig.write_html('C:/Screenshot_Network_Location/correlation_svg_curved_clusters.html')
